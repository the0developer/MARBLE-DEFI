...
import db, { PoolsTokens } from '../store/RefDatabase';
...
import { registerTokensAction } from './creators/token';
...
import { getExplorer, ExplorerType } from './device';
import {
  STABLE_POOL_ID,
...
export const getLiquidity = (
  ...
  const amount1 = toReadableNumber(tokenIn.decimals, pool.supplies[tokenIn.token_address]);
...
export const getPoolsByTokens = async ({
  ...
  if (loadingTrigger || (!cacheTimeLimit && cacheForPair)) {
    ...
    filtered_pools = filtered_pools.filter(
      (p) => {
        return p.supplies[tokenInId] && p.supplies[tokenOutId]
      }
    );
...
export const removeLiquidityFromPool = async ({
  ...
  for (let i = 0; i < tokenIds.length; i++) {
    ...
    const ftBalance = await ftGetStorageBalance(tokenId, accountId);
...
export const removeLiquidityFromStablePool = async ({
  ...
  for (let i = 0; i < tokenIds.length; i++) {
    ...
    const ftBalance = await ftGetStorageBalance(tokenId, wallet.getAccountId());
    if (ftBalance === null) {
      withDrawTransactions.unshift({
        receiverId: tokenId,
        functionCalls: [
...
export const addSimpleLiquidityPool = async (
  ...
  const storageBalances = await Promise.all(
    tokenIds.map((id) => ftGetStorageBalance(id, CONTRACT_NAME))
  );
