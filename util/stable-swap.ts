...
export const estimateSwap = async ({
  ...
  try {
    ...
    throw new Error(
      `${intl.formatMessage({ id: 'no_pool_available_to_make_a_swap_from' })} ${tokenIn.symbol
...
export const instantSwapGetTransactions = async ({
  ...
  if (wallet.isSignedIn()) {
    const tokenOutRegistered = await ftGetStorageBalance(tokenOut.id, wallet.getAccountId()).catch(
      () => {
