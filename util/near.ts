...
import { WALLET_TYPE } from './sender-wallet'
...
export const refFarmViewFunction = ({
  ...
  // create wallet connection
  const wallet = new SpecialWallet(
    near,
    process.env.NEXT_PUBLIC_FARM_CONTRACT_NAME
  )
  return wallet
    .account()
    .viewFunction(process.env.NEXT_PUBLIC_FARM_CONTRACT_NAME, methodName, args)
}
...
export const nearViewFunction = ({ tokenAddress, methodName, args }) => {
  const { keyStores } = nearAPI
  const keyStore = new keyStores.BrowserLocalStorageKeyStore()
  const config = {
    networkId: process.env.NEXT_PUBLIC_NODE_URL,
  ...
  // create wallet connection
  const wallet = new SpecialWallet(near, tokenAddress)
  return wallet.account().viewFunction(tokenAddress, methodName, args)
}
