...
import { STABLE_POOL_ID, STABLE_TOKEN_IDS, isStablePool } from './near'
import { Pool, getStablePoolInfoKey } from './pool'
import { getSwappedAmount, estimateSwap } from './stable-swap'
...
export const calculateFeeCharge = (fee: number, total: string) => {
  return math.floor(math.evaluate(`(${fee} / ${BPS_CONVERSION}) * ${total}`), 3)
}
