...
import { isNotStablePool, PoolDetails } from '../util/pool';
...
class RefDatabase extends Dexie {
  ...
  public constructor() {
    ...
    this.version(5.4).stores({
      ...
      farms: 'id, pool_id, apr, userStaked, userUnclaimed_reward, reward_per_session, session_interval, start_at, withdraw_interval, seed_id',
      // farms: 'pool_id, start_at, reward_per_session, session_interval',
      pools_tokens:
  ...
  public async cacheFarms(farms: any) {
    ...
    const farmsInfo = farms.map(farm => ({
      ...
      apr: farm.apr,
      reward_per_session: farm.reward_per_session,
      session_interval: farm.session_interval,
      start_at: farm.start_at,
      userStaked: farm.userStaked,
      userUnclaimedReward: farm.userUnclaimedReward,
      withdraw_interval: farm.withdraw_interval,
